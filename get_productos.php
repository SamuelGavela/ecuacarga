...
<body>
    <?php
        ...
        $array_result = lista_productos($conexion2, $p_cliente);
       
    ?>
    
        <select id="producto_01" name="producto_01" class="col-md-6 form-control" onchange="showSubProductos('<?php printf($p_cliente); ?>', this.value)">
                <option value="">Selec. Producto</option>
            <?php 
                for ($c_i = 0; $c_i < count($array_result); $c_i++) 
                { 
            ?>
                <option value="<?php printf($array_result[$c_i]['nom_producto']); ?>"><?php printf(utf8_encode($array_result[$c_i]['desc_producto'])); ?></option>
            <?php 
                } 
            ?>
        </select> 
