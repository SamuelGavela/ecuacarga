    ...
    <script type="text/javascript" language="javascript" >
        $(document).ready(function() {
            ...
            $("#bt_cargar").click(function() {
					  ...
					  form_planes_01 = document.getElementById('planes_01').value;
					  form_sub_planes_01 = document.getElementById('sub_planes_01').value;
					...
					/*  if (form_planes_01 == ""){
							Lobibox.notify('error', {
								...
								msg: 'Debe seleccionar un PLAN para continuar' 
							}); 
					  ...
					  if (form_sub_planes_01 == ""){
							Lobibox.notify('error', {
								img : './imagenes/icono_error.png',
								sound: false,
								title: 'DATOS INCOMPLETOS',
								msg: 'Debe seleccionar un SUBPLAN para continuar' 
							}); 
							document.getElementById("demo").innerHTML = "Error al cargar Archivo: Datos Incompletos";
							$("#bt_cargar").attr("disabled", false);
							validacion_ok = false;
                      ...
                      if (file && validacion_ok){
							  ...
							  form_data.append("form_planes_01", form_planes_01);
							  form_data.append("form_sub_planes_01", form_sub_planes_01);
							  form_data.append("form_fecha_01", form_fecha_01);
                              
                              $.ajax({
                                url: "cargar_archivo_nova.php", // Upload Script
                                cache: false,
                                contentType: false,
                                processData: false,
                                data: form_data,
                                ...
                                success: function(response) {
                                    ...
                                    if ( n >= 0 ){
										try {
												...
												document.getElementById("demo").innerHTML = "Archivo Cargado. "+response;
												$("#bt_cargar").attr("disabled", false);

												// INICIO CARGA LISTADO
												if ( $.fn.DataTable.isDataTable('#nova_cab-grid') ) {
														$('#nova_cab-grid').DataTable().clear().destroy();
												}

												var dataTable = $('#nova_cab-grid').DataTable( {
													"processing": true,
													"serverSide": true,
													"bFilter": true,
													"retrieve": true,
													"ajax":{
														url :"nova_cab-grid-data.php", // json datasource
														type: "post",  // method  , by default get
														error: function(e){  // error handling
															//console.log('error '+e);
															$(".nova_cab-grid-error").html("");
															$("#nova_cab-grid").append('<tbody class="nova_cab-grid-error"></tbody>');
															$("#nova_cab-grid_processing").css("display","none");
														}
													}
												});
												refrescar = function() {
                                        ...
                                        Lobibox.notify('error', {
                                            ...
                                            msg: 'Ver detalle del error en la seccion Resultado de la Carga del Archivo' 
                                        }); 
										console.log("ERROR: "+res); 
										document.getElementById("demo").innerHTML = "<strong>ERROR #1 al cargar Archivo: </strong></br>"+response;
										$("#bt_cargar").attr("disabled", false);
										removeLoading();			
                                    }
                                ...
                                error: function (xhr, ajaxOptions, thrownError) {
									...
									document.getElementById("demo").innerHTML = "<strong>ERROR #2 al cargar Archivo: </strong></br>"+response;
									$("#bt_cargar").attr("disabled", false);
                                    alert(thrownError);
                                }
		...
		$(window).load(function(){
			// CARGA INICIAL CUANDO INICIE LA PAGINA
			if ( $.fn.DataTable.isDataTable('#nova_cab-grid') ) {
					$('#nova_cab-grid').DataTable().clear().destroy();
			}
		...
		function ver_detalle_nova(trama){
			var window_editar = window.open('show_detalles_archivo_nova.php?p_trama='+trama,'fr_detalle');
		}
		
        function eliminar_nova(registro_encriptado){
			var parametros = {
			...
			if ( registro_encriptado != "" ) {
				$.ajax({
						...
						success:  function (response) {
							...
							$('#nova_cab-grid').DataTable().ajax.reload(null, false);
        ...
        function actualizar_nova_pend_activo(registro_encriptado){
			form_usuario = document.getElementById('form_usuario').value;
			var_estado_actual = "P";
			var_estado_nuevo = "A";
			var_observacion = "Pendiente de Facturar";

			var parametros = {
			  "p_trama" : registro_encriptado,
			  "p_usuario" : form_usuario,
			  "p_estado_actual" : var_estado_actual,
			  "p_estado_nuevo" : var_estado_nuevo,
			  "p_observacion" : var_observacion
			};			
			
			if ( registro_encriptado != "" ) {
				$.ajax({
						data:  parametros,
						url:   'actualizar_estados_nova.php',
						type:  'post',
						success:  function (response) {
							Lobibox.notify('success', {
								img : './imagenes/icono_ok.png',
								...
								title: 'EXITO',
								position: 'right top',
								msg: 'El registro ha sido Grabado como Activo exitosamente.'
							});
							$('#nova_cab-grid').DataTable().ajax.reload(null, false);
...
<body>
                   ...
                   <div class="row"  style="background-color: none;">
        				...
        				<div class="col-sm-12 col-md-12" style="background-color: none;">
                            <section id="content">
                                    ...
                                    <header>
										<span style="color:#333333; font-size:21px; padding-left: 5px;"><strong>Carga de Archivos NOVA</strong></span>
									</header>
									</br>	
									<table class="table table-bordered" style="font-size:14px;">
										...
										<tbody>
											<tr>
												<td>
													    <select id="planes_01" name="planes_01" class="col-md-6 form-control" onchange="showSubPlanes(this.value)">
															<option value="">Selec. Plan</option>
														<?php 
															$array_result = lista_planes_nova($conexion);
															for ($c_i = 0; $c_i < count($array_result); $c_i++) 
															{ 
														?>
																<option value="<?php printf($array_result[$c_i]['cod_plan']); ?>"><?php printf($array_result[$c_i]['nombre_plan']); ?></option>
														<?php 
															} 
														?>
													</select>   
												...
												<td><input type="text" id="fecha_01" name="fecha_01" ng-model="codigo" ng-change="#"  maxlength="15" value="" placeholder="yyyy-mm-dd" class="form-control"  autocomplete="off" /></td>
												<td>
									...
									<div id="tabla_resultados_carga" style="width:100%; height:150px; background-color:#E1E1E1;overflow-y: scroll;"> 
										<span style="color:#FF0000; font-size:16px; padding-left: 5px;"><strong>Resultado de la Carga del Archivo</strong></span>
										<p id="demo" style="padding-left: 10px; padding-right: 5px;  font-size:12px;"></p>	
									</div>
									</br>							
									<div id="tabla_archivos_cargados"> 
										<span style="color:#FF0000; font-size:17px; padding-left: 5px;"><strong>Archivos Cargados y Pendientes de Grabar</strong>&nbsp;&nbsp;<button type="button" id="bt_consultar" class="btn btn-success btn-xs">&nbsp;&nbsp;<span class="fa fa-search"></span>&nbsp;Refrescar Consulta&nbsp;&nbsp;</button></span>	
										<p id="demo_tabla_archivos">
											<table id="nova_cab-grid" cellpadding="0" cellspacing="0" border="0" class="display" width="100%" style="font-size: 10px;">
												<thead>
													<tr>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>&nbsp;</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>ID</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>ARCHIVO CARGADO</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>PLAN</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>SUB-PLAN</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>FEC. ARCHIVO</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>CANT. REGISTROS</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>TOTAL DEBITOS</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>USUARIO</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>DETALLE</p></th>
														<th><p style='text-align: center; padding:0px; margin: 0px; color:red;'>GRABAR</p></th>
													</tr>
												</thead>
											</table>
										</p>	
	...
	<input type="hidden" id="form_usuario" name = "form_usuario" value="<?php printf($_SESSION['user_ecucarga']); ?>" />
    <!--FIN CUERpo -->
	...
	<script type="text/javascript">    
		...
		function showSubPlanes(str)
		{
			if (str=="")
			{
				document.getElementById("div_sub_planes").innerHTML="";
				return;
			} 
			...
			xmlhttp.onreadystatechange=function()
			{
				...
				{
					document.getElementById("div_sub_planes").innerHTML=xmlhttp.responseText;
				}
			}
			xmlhttp.open("GET","get_sub_planes.php?p_cod_plan="+str,true);
			xmlhttp.send();
		}
	</script>								  
